            ...
            {{-- <form class="max-w-md mx-auto my-4">
                ...
                <div class="relative">
                    ...
                    <input type="search" id="default-search"
                        ...
                        placeholder="Temukan ruangan disini..." required />
                    <button type="submit"
                ...
                <table id="dataTable" class="w-full border-separate border-spacing-0 text-sm text-black">
                    <thead class="bg-gray-200 text-gray-800">
                        <tr>
                            ...
                            <th class="p-2 text-center">Nama Ruangan</th>
                    ...
                    <tbody class="bg-white text-center" id="dosenTableBody">
                        @foreach ($ruangan as $index => $item)
                            <tr class="border-b border-gray-200">
                                <td class="p-2">{{ $item->kd_ruangan }}</td>
                                <td class="p-2">{{ $item->nama_ruangan }}</td>
                                <td class="p-2">{{ $item->kapasitas }}</td>
                                <td class="p-2">
                                    ...
                                    <form id="delete-form-{{ $item->id }}"
                                        action="{{ route('adminRuangan.destroy', $item->id) }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                    ...
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div class="p-6 text-center">
                                            ...
                                            <form action="{{ route('adminRuangan.update', $item->id) }}" method="POST"
                                                ...
                                                @method('PUT')
                                                <div class="text-left">
                                                    <label for="kd_ruangan"
                                                        class="block text-sm font-medium text-gray-900">Kode Ruangan</label>
                                                    <input type="text" name="kd_ruangan" id="kd_ruangan"
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1"
                                                        placeholder="Masukkan Kode Ruangan"
                                                        value="{{ $item->kd_ruangan }}">
                                                </div>
                                                <div class="text-left mt-4">
                                                    <label for="nama_ruangan"
                                                        class="block text-sm font-medium text-gray-900">Nama Ruangan</label>
                                                ...
                                                <div class="text-left mt-4">
                                                    <label for="kapasitas"
                                                        class="block text-sm font-medium text-gray-900">Kapasitas</label>
                                                    <input type="text" name="kapasitas" id="kapasitas"
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1"
                                                        placeholder="Masukkan Jumlah Kapasitas Ruangan"
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow-lg">
                    <div class="flex items-center justify-between p-4 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-black">Tambah Ruangan</h3>
                        <button type="button"
                    ...
                    <form action="{{ route('adminRuangan.store') }}"method="POST" class="p-4">
                        @csrf
                        <div class="text-left">
                            <label for="kd_ruangan" class="block text-sm font-medium text-gray-900">Kode Ruangan</label>
                            <input type="text" name="kd_ruangan" id="kd_ruangan"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1"
                                placeholder="Masukkan Kode Ruangan" required>
                        </div>
                        <div class="text-left mt-4">
                            <label for="nama_ruangan" class="block text-sm font-medium text-gray-900">Nama Ruangan</label>
                            <input type="text" name="nama_ruangan" id="nama_ruangan"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1"
                                placeholder="Masukkan Nama Ruangan" required>
                        </div>
                        <div class="text-left mt-4">
                            <label for="kapasitas" class="block text-sm font-medium text-gray-900">Kapasitas</label>
                            <input type="text" name="kapasitas" id="kapasitas"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1"
                                placeholder="Masukkan Jumlah Kapasitas Ruangan" required>
                        </div>
                        <div class="flex justify-end">
                            <button type="submit"
                                class="bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 transition duration-300 font-medium text-sm my-2">
        ...
        {{-- @if ($ruangan->total() > 5)
            <div class="flex flex-col items-center my-6">
                <span class="text-sm text-gray-700 dark:text-gray-400">
                    Menampilkan <span
                        class="font-semibold text-gray-900 dark:text-white">{{ $ruangan->firstItem() }}</span>
                    sampai
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $ruangan->lastItem() }}</span> dari <span
                        class="font-semibold text-gray-900 dark:text-white">{{ $ruangan->total() }}</span> ruangan
                </span>
                <div class="inline-flex mt-2 xs:mt-0">
                    <button {{ $ruangan->onFirstPage() ? 'disabled' : '' }}
                        class="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-gray-800 rounded-s hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
                        {{ $ruangan->previousPageUrl() ? 'onclick=window.location.href=\'' . $ruangan->previousPageUrl() . '\'' : '' }}>
                        Sebelumnya
                    </button>
                    <button {{ !$ruangan->hasMorePages() ? 'disabled' : '' }}
                        class="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-gray-800 border-0 border-s border-gray-700 rounded-e hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
                        {{ $ruangan->nextPageUrl() ? 'onclick=window.location.href=\'' . $ruangan->nextPageUrl() . '\'' : '' }}>
                        Selanjutnya
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function confirmDelete() {
            Swal.fire({
                ...
                text: "Anda tidak akan dapat mengembalikan ruangan ini!",
                icon: 'warning',
